...
import { type addFixedInflowSchema } from "~/server/schemas/inflows";

const BRL = Make.TYPES.BRL;
Make.CONFIGURE({ money: BRL });

type AddFixedInflowProps = {
  isOpen: boolean;
  onSubmit?: () => void;
  onClose?: () => void;
  onOpenChange?: () => void;
...
type FixedExpense = z.infer<typeof addFixedInflowSchema>;
...
export const AddFixedInflow: FunctionComponent<AddFixedInflowProps> = ({
  isOpen,
  onOpenChange,
  onClose,
}) => {
  ...
  const addFixedInflow = api.inflows.addFixedInflow.useMutation();
  const utils = api.useContext();

  const onSubmit = (data: FixedExpense) => {
    addFixedInflow.mutate(
      {
      ...
      },
      {
        onSuccess() {
          ...
          toast("Entrada fixa cadastrada com sucesso", {
            type: "success",
          });
          utils.inflows.getAllFixedInflows.invalidate();
        },
      }
    );
  };
        ...
        {(onClose) => (
          <>
            ...
            <ModalFooter>
              ...
              <Button
                color="success"
                isLoading={addFixedInflow.isLoading}
